...
ibv_mr *createMemoryRegion(uint64_t mm, uint64_t mmSize, RdmaContext *ctx) {
  ...
  mr = ibv_reg_mr(ctx->pd, (void *)mm, mmSize,
                  IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ |
                      IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_ATOMIC | IBV_ACCESS_ON_DEMAND);

  if (!mr) {
    Debug::notifyError("Memory registration failed");
    Debug::notifyError("error:%s",strerror(errno));
  }
...
ibv_mr *createMemoryRegionOnChip(uint64_t mm, uint64_t mmSize,
  ...
  struct ibv_mr *mr = ibv_reg_mr(ctx->pd,(void *)mm, 
                          mmSize, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ |
                          IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_ATOMIC);
  ...
  ibv_memcpy_to_dm(dm,0,(void *)buffer,mmSize);
